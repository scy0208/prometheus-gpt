...
import { Trash, Logout, PlayerStop, ThumbUp, ThumbDown } from 'tabler-icons-react';
...
const Chat: FC<Props> = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let updatedConversation: Conversation = {
      ...selectedConversation,
      messages: [...selectedConversation.messages, { id: userMessageId, role: "user", content: message }]
    };
    ...
    const httpResponse = await fetch('/api/gpt-stream-api', {
      ...
      body: JSON.stringify({
        ...
        username: user,
        message,
        dialogues: updatedConversation.messages.map(({ id, ...message }) => message)
      }),
    })
  ...
  const handleClaudeResponse = async (controller: AbortController, httpResponse: Response, updatedConversation: Conversation) => {
    ...
    while (!done) {
      ...
      if (chunkValue != '') {
        updatedConversation = {
          ...updatedConversation,
          messages: [...updatedConversation.messages.slice(0, -1), { id: uuId, role: "assistant", content: chunkValue }]
        }
        setSelectedConversation(updatedConversation)
      }
    }
  ...
  const renderMessages = (conversation: string, item: MessageWithId) => {
    if (item.role === "user") {
      return (
